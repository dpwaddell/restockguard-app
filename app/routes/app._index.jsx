...
export default function Index() {
  ...
  return (
    <s-page heading={`${getGreeting()}, ${storeName(shopDomain)}`}>
      ...
      {subscriberCount === 0 && (
        <div style={{ backgroundColor: "#fff", border: "1px solid #e1e3e5", borderRadius: "10px", padding: "24px", marginBottom: "16px" }}>
          <div style={{ fontSize: "16px", fontWeight: "700", color: "#202223", marginBottom: "20px" }}>Get set up in 3 steps</div>
          {[
            ...
            <div key={step} style={{ display: "flex", alignItems: "center", gap: "16px", padding: "14px 0", borderBottom: step < 3 ? "1px solid #f1f2f3" : "none" }}>
              <div style={{ width: "32px", height: "32px", borderRadius: "50%", backgroundColor: "#1a56db", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "700", flexShrink: 0 }}>{step}</div>
              <span style={{ flex: 1, fontSize: "14px", color: "#202223" }}>{text}</span>
              <button onClick={() => external ? window.open(href, "_blank") : navigate(href)} style={{ fontSize: "14px", color: "#1a56db", fontWeight: "600", background: "none", border: "none", cursor: "pointer", padding: 0 }}>Go →</button>
            </div>
          ))}
        </div>
      )}
